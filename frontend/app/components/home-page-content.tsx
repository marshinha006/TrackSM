...
import RecentBannerCarousel from "./recent-banner-carousel";
...
function renderCard(item: TmdbItem, mediaType: "filme" | "serie") {
  const title = item.title ?? item.name ?? "Sem titulo";
  const fillPercent = Math.max(0, Math.min(100, item.vote_average * 10));

  return (
    <Link className="card-link" href={`/detalhe/${mediaType}/${item.id}`} key={`${mediaType}-${item.id}`}>
...
export default async function HomePage() {
  try {
    ...
    const recentItems = [
      ...recentMovies.map((item) => toRecentItem(item, "Filme", "filme")),
      ...recentSeries.map((item) => toRecentItem(item, "Serie", "serie")),
    ]
      ...
      <>
        ...
        <main>
          ...
          <section>
            ...
            <div className="grid">{popularMovies.slice(0, 15).map((item) => renderCard(item, "filme"))}</div>
          </section>

          <section>
            <h2 className="section-title">Series populares</h2>
            <div className="grid">{popularSeries.slice(0, 15).map((item) => renderCard(item, "serie"))}</div>
          </section>
        </main>
      </>
    );
